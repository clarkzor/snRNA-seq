# Quality Control & Annotation: Xenopus tropicalis gastrulation timecourse snRNA-seq
...
├── validation/              # repository-level notebook validation reports
├── environment.yml
└── requirements.txt
```
...
in `data/README.md`, then run each notebook from top to bottom. The notebooks
resolve all paths from the repository root, so `data/` and `results/` must be
siblings of `notebooks/`, never subdirectories inside it.

Validate the project layout and notebook files before running an analysis:
...
The notebook validator writes CSV, JSON, and text reports to the top-level
`validation/` directory. These checks confirm valid Jupyter notebook structure
and static Python syntax; they do not execute the Scanpy analyses.
...
├── qc/                      # Section 1 and post-doublet QC figures/tables
├── figures/
...
- Section 1 QC figures, Scrublet histograms, and post-doublet read-count summaries;
...
Raw matrices and generated `.h5ad` files are intentionally excluded from
Git. The published Stage 10 unfiltered object should be referenced with an
external data-repository link in `data/README.md`; unpublished stage inputs
are not distributed here. The repository tracks code, configuration,
documentation, and lightweight audit tables. Selected publication-quality figures can be copied to
a dedicated tracked manuscript directory if needed.
